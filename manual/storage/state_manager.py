...
class StateManager:
    ...
    def _reconstruct_event_stats(self, stats_dict: dict) -> EventStats:
        ...
        All-time totals (wins/losses/gems/packs/plays/draws/milestones) are
        computed properties/methods on EventStats derived from recent_runs,
        not stored fields - so a save file from before those totals existed,
        or before recent_runs was uncapped, just needs its recent_runs list
        reconstructed correctly and the numbers fall out automatically via
        _known_fields() dropping any stale stored keys.
        """
