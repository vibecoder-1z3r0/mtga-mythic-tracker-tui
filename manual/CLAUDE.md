...
- Trends grew a "Runs:" row: the last 10 completed runs' win counts (most
  recent leftmost, same ordering as the games glyphs), each number colored
  on a straight red-to-green gradient via `_gradient_color(wins/win_cap)`
  (`rgb(255*(1-ratio), 255*ratio, 0)`). `_run_result_emoji(run, event)`
  (🏆/🔥/💰/✅/😐/😢/💀 result-quality tiers, evaluated best-to-worst so
  the highest-applicable tier wins on a boundary collision - 💰 in
  particular means prize *gems alone*, packs excluded, cover the entry
  cost, a literal "this run paid for itself" breakeven rather than a
  fixed percentage) is still defined and still covered by a regression
  test, but is currently unused in the UI - disabled in favor of the
  color gradient per a later request, kept intact in case it comes back.
  This whole feature went through several rounds of live back-and-forth
  before landing on the color-gradient version (worth remembering: don't
  implement a visual/design choice like this speculatively - confirm the
  exact spec first, since guessing wrong here cost a full implement-
  revert-reimplement cycle on the emoji table alone).
- Trends layout: `Runs:` now comes first (blank line separating it from
  the two rows below), and the per-game result row was renamed `Games:`
  (from `Results:`) now that "Runs" is a distinct concept in the same
  section - `Results:` alone was ambiguous between game results and run
  results once both existed. The `"(Last 10 Games)"` header suffix was
  dropped as redundant - the row labels already say what's being shown.
